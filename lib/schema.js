...
/**
 * Fetch the content of all subdocument fields and return in a new object
 *
 * @param {Object} obj
 * @param {Function} callback
 */
function getChildren(schema, obj, callback){
  ...
  function each(key, id, next){

    if( schema.fields[key].type != types.subschema ){
      next();
      return;
    }

    subschema = schema.fields[key].schema;
  ...
  loopDocumentFields(schema, obj, each, end);
}
...
/**
 * Create a new async loop for the fields of given doc.
 *
 * @param {Schema} schema
 * @param {Document} document
 * @param {Function} each
 * @param {Function} end
 */
function loopDocumentFields(schema, document, each, end){
...
function get(schema, query, callback){
  ...
  schema.driver.get(schema.collection, query, function(error, result){
    ...
    logging.debug('Found result for %s: "%s"', query, result);
...
function save(schema, doc, callback){
  ...
  saveChildren(schema, doc, function(error, replacements){
    ...
    schema.driver.save(schema.collection, content, function(error, result){
...
function saveChildren(schema, document, callback){
  ...
  function each(field, value, next){
    ...
    subschema = value.id.schema;

    save(subschema, value, function(error, id){
  ...
  loopDocumentFields(schema, document, each, end);
...
function set(schema, query, replacement, callback){
  logging.info('Setting %s in %s as %s', query, schema, schema, replacement);
