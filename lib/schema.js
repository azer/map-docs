 ...
 * @param {Function} callback
 */
function insert(schema, doc, callback){

  logging.info('Inserting given docs %s to "%s".', JSON.stringify(doc), schema);
  ...
  saveUnsavedChildren(schema, doc, function(error, replacements){
  ...
  (function eachdoc(i, error){

    if( i >= docs.length ){
      callback(undefined, listInst ? results : results[0]);
      return;
    }
    ...
    function savedoc(){
      ...
      schema.driver.insert(schema.collection, content, function(error, results){
        ...
        results.push( newDocument(schema, results) );
...
/**
 * Insert given single document.
 *
 * @param {Schema} schema
 * @param {Document} doc
 */
function insertDocument(schema, doc){
 ...
 * @param {Function} callback
 */
function saveUnsavedChildren(schema, document, callback){
  ...
  (function each(i){
    ...
    console.log(33, field, isDocument(document[field]));
